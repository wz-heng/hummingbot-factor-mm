...
| §6.5 (real exchange server time) | Wire `_build_snapshot`'s `exchange_ts_sec` to a periodically cached `binance.timev3` REST call | Stub returns `now`; clock-drift check always passes — verify and replace at M4 before declaring testnet live |
